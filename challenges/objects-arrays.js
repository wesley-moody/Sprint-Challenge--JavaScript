...
const universities = [];
  let newArr = graduates.map(function(currentValue){
    return universities.push(currentValue.university);
  });
    universities.sort((uniA,uniB) => (uniA >uniB) ? 1 : -1)
...
*/
const displayNames = [];
zooAnimals.forEach(function(fuzzies) {
  return displayNames.push(`Name: ${fuzzies.animal_name}, Scientific: ${fuzzies.scientific_name}`);
});
...
zooAnimals.filter(function(few) {
